...
variable "atlas_provider_name" {
  type        = string
  default     = "AWS"
  description = "Cloud that hosts the Atlas cluster: AWS, GCP, or AZURE. This is not the cloud that runs Kubernetes."

  validation {
    condition     = contains(["AWS", "GCP", "AZURE"], var.atlas_provider_name)
    error_message = "Atlas provider must be AWS, GCP, or AZURE."
  }
}

variable "atlas_region_name" {
  type        = string
  description = "Atlas region name, for example EU_CENTRAL_1. AWS callers pass the AWS region with hyphens replaced by underscores and uppercased."
}

variable "ip_access_cidrs" {
  type        = list(string)
  description = "CIDRs allowed to reach the cluster. AWS passes the VPC CIDR. UpCloud passes the NAT gateway public IP as a /32."

  validation {
    condition     = length(var.ip_access_cidrs) > 0
    error_message = "At least one IP access CIDR is required."
  }
}
...
variable "is_database_storage_encrypted" {
  type        = bool
  description = "Use a customer-managed key from atlas_provider_name for encryption at rest. The Atlas project must already have that key configured. Atlas still encrypts disks with its own keys when this is false."
  default     = false
}

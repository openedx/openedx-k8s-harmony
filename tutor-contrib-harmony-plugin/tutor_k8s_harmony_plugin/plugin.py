...
config = {
    "defaults": {
        "VERSION": __version__,
        # This plugin assumes you are using Traefik as an ingress controller to provide
...
if FORUM_ENV:
    ...
    def _add_forum_env_vars(env_vars: dict):
        ...
        return {
            **env_vars,
            "SEARCH_SERVER": "{{ ELASTICSEARCH_SCHEME }}://{{ (K8S_HARMONY_SEARCH_CLUSTER_HTTP_AUTH + '@') if K8S_HARMONY_SEARCH_CLUSTER_HTTP_AUTH }}{{ ELASTICSEARCH_HOST }}:{{ ELASTICSEARCH_PORT }}",
